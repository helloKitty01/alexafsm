...
| [docs/kv-cache-discussion/](./docs/kv-cache-discussion/) | KV cache 命中率与 agent 上下文工程（分层排布、渐进式加载、压缩策略）的设计讨论；含 v1 整合汇报 9 页、v2 汇报大纲（13）、实测数据需求（14）与逐调用实测 `token_breakdown.csv`、v2 幻灯片 P1 组成对比 v1.2 / P2 尺子与三方案 / P7 自动化统计 |
