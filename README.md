...
| [docs/kv-cache-discussion/](./docs/kv-cache-discussion/) | KV cache 命中率与 agent 上下文工程（分层排布、渐进式加载、压缩策略）的设计讨论；含**最终汇报版承载 `kv cache方案（汇报版）.v1.3`（8 页 + 附录 3 页，实测 + 推演；P1 组成对比 + 三尺子，P3–P5 任务 / 问答 / 闲聊逐 Q 账目按 v7.2 版式当前实测 vs 目标推演对照，已上线第一步实测与预热下沉附录）**、v2 汇报大纲（13）、实测数据需求（14）、逐调用实测 `token_breakdown.csv`、prompt 各段内容变化对照，以及 v2 单页幻灯片 P1 组成对比 v1.2 / P2 尺子与三方案 / P7 自动化统计；上一版整合材料 kvcache整体方案.v1 暂留对照 |
