...
| [docs/kv-cache-discussion/](./docs/kv-cache-discussion/) | KV cache 命中率与 agent 上下文工程（分层排布、渐进式加载、压缩策略）的设计讨论；含 v1 整合汇报 9 页、v2 汇报大纲（13）、实测数据需求（14）与 v2 P1 上下文组成对比单页 v1.1（Tools → SP → UAT，四级分级 × UAT 按 Q/loop 展开矩阵） |
