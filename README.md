...
| [docs/openclaw-automation/](./docs/openclaw-automation/) | openClaw 自动化任务统一方案（定时 + 事件订阅 + 模型判定触发，v2.3 幻灯片 30 页五章结构 + `02-event-catalog.md` 事件目录全文，v0.1–v2.2 留档 history/）：以 openClaw cron service 为唯一中枢按需新增——四种触发（时间 / 脚本轮询 / 模型判定轮询 / 外部事件）一条执行路径 **tick → trigger → fire → limits → run**（v2.3 新增 P6 给 tick / trigger / fire / limits / run / filter / nextCheckAt / state / 派生事件九个名词正式定义与命名约定）；原生 `cron` 工具全貌（一个工具，`schedule.kind ∈ at / every / cron / stream / event / multi`，`trigger.kind ∈ script / agent`，job 级 `limits`）、session 四选一、事件中心（EventHub）/ 原生 / 新增三方归属与 γ 方案、谁来订阅（Gateway 内部完成，模型不感知 debounce / staleAfter）、`schedule.kind="event"` 规格（模型只写 eventType + filter）与投递语义、事件入口（传输四选一 + `onTick → runJob` 点火路径）、工具可见范围 × 订阅生命周期（`event_catalog` 只读 / 四个填槽解析工具 / `phone_state.get` / `event_publish` 主 loop 不可见 / 判定阶段只读 + `trigger_result` 收口）、两页端到端时序、上下文组织（五块信封）、七步编译与三套模板（message 三段 / 判定 prompt 三段 / 组合脚本 T1–T3）、`trigger.kind="agent"` 规格（`{fire,message,state,nextCheckAt?}` 契约、六条约束、三层级联门 + `nextCheckAt` 自设闹钟、S9 六 tick 演进）、十二个场景全表（v2.3 补 S10–S12 各对应组合模板 T1 / T2 / T3）、派生事件"结果只给自己用还是给别人用"（v2.3 重写：路 A 即普通 job，路 B 只是 payload 换 publish + `publishes` 登记目录，2×2 例子 + JSON 逐字段对比 + 两层 limits）、可靠性清单与预授权等开放问题；**v2.2 三处收敛：guards 废除 → job 级 `limits {activeWindow, cooldown, maxRunsPerDay, once}` 归 cron service（G4 改为"filter 归事件中心，限额与组合归 cron service"，事件中心不做 CEP）；事件成为 tick 源（`multi.sources[]` 混排事件与时间，五类组合条件——单事件布尔 / 跨事件或 / 事件且状态 / 时序且 / 缺席非——全部用 trigger state + `nextCheckAt` 落地）；事件目录定稿（条目 schema 六项可见 + 四项 internal、filter 单事件布尔语法、10 组 63 条内建事件静态进编译期 prompt）**；第五章「模型视角」P25–P30 同步到新契约（tick 信封、self tick、"已定 vs 倾向"）；30 页逐页校验无溢出 |
