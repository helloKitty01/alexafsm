...
| [docs/kv-cache-discussion/](./docs/kv-cache-discussion/) | KV cache 命中率与 agent 上下文工程（分层排布、渐进式加载、压缩策略）的设计讨论；含**最终汇报版 `kv cache方案（汇报版）.v3.3`（3 页，给领导：P1 数据基线——优化前 / 优化后两张三列表（序号 · 模块 · 长度），Tools 4→1、动态 SP 内→U 前半两行蓝底，右栏业务占比 47/28/25 + 完整 16 桶会话轮数分布（1Q 51.5% … 100+ 0.15%）；P2 上下文组成对比——两条结构条 + 三步改造一行 + 当前 / 目标两张 6 行逐次调用矩阵（右列"白算 + 必算 = 实算"），底部两行：账目逻辑（输入 = 已缓存 + 白算 + 必算 / 实算 = prefill 真正计算的量 / cache 命中率 = 已缓存 ÷ 输入）与目标衡量（白算 → 0 验收线 97K→0、必算尽可能小 39.6K 两边相同、实算 137K→40K −71%、命中率 62%→89%）；P3 轮数 × 业务——ΔS(1)=5.9、ΔS(n≥2)=14.4+7.1(n−1)，Q1–Q6 实算曲线与每轮命中率（当前 74%→60%、目标 81%→95%，口径统一"长会话单轮到 95% 左右"），首调 TTFT 预估（参考实测 46 ms/K：Q2–Q12 1.1–4.4 s → 150 ms），分桶 1Q–5Q / 6–10Q / 11Q+（≥11 轮按 12 轮计）加权 −66%、命中率 66%→91%，三句话含会话级预热的收益（感知 TTFT −40%）与代价（算力 +10%、TTL）。两边同内容只换排布、高频 skill 不进 SP、动态两边 3K、query 0.1K、A 拆 tool call / answer、T 分 webSearch 10 / SkillLoad 5 / 工具 2.5；页内 `BASELINE` 数据绑定，三视口校验通过；v3.2 / v3.1 / v3、升级版 v1 / v2、v1.4 及更早留档 history/，archify 源仍在 archify/）**、v2 汇报大纲（13）、实测数据需求（14）、旧基线（15）、**当前工作基线 16 优化后 / 17 优化前（各一张四列表、长度全为确定数且两边同长）、v3.3 账目与公式底稿（18）**、工作底稿 M1–M9 长度与变化行为表、基线模板 v2（0921 实测 chars，现网中间态）、逐调用实测 `token_breakdown.csv`、prompt 各段内容变化对照，以及 v2 单页幻灯片 P1 组成对比 v1.2 / P2 尺子与三方案 / P7 自动化统计；上一版整合材料 kvcache整体方案.v1 暂留对照 |
