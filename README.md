...
| [docs/openclaw-automation/](./docs/openclaw-automation/) | openClaw 自动化任务统一方案（定时 + 事件订阅，v0.4 幻灯片 15 页，v0.1–v0.3 留档 history/）：以 openClaw cron service 为唯一中枢按需新增——原生 `cron` 工具全貌与层级图（一个工具，`at / every / cron / stream / event` 是 schedule.kind）、session 四选一、事件中心（EventHub）/ 原生 / 新增三方归属与 γ 方案、谁来订阅（Gateway 内部完成）、新增 `schedule.kind="event"` 规格与投递语义、工具可见范围 × 订阅生命周期（`event_catalog` 只读 / `event_publish` 主 loop 不可见）、两页端到端时序（S3 到家 12 步 + run 状态机；S2 路 B 两条 job 接力）、事件启动 Agent 的上下文组织（五块信封）、`payload.message` 三段编译模板、三种触发的完整 `cron add` 工具调用实例、派生事件路 A / B 与两类生产者、可靠性清单 |
