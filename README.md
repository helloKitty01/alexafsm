...
| [docs/openclaw-automation/](./docs/openclaw-automation/) | openClaw 自动化任务统一方案（定时 + 事件订阅 + 脚本 / 模型判定触发；**v3.0 收敛版**：主稿 16 页 + 附录 10 页 + `02-event-catalog.md` 事件目录全文，v0.1–v2.3 留档 history/）：以 openClaw 原生 `cron` 为唯一中枢按需新增——一条路 **tick → trigger → fire → limits → run**、三道门各答一个问题（filter 事件中心 / trigger checker / limits cron service）；**新增只有五项**（`schedule.kind="event"` + `sources[]` 与事件中心适配器、tick + `nextCheckAt` self 闹钟、job 级 `limits {activeWindow, cooldown, maxRunsPerDay, maxRunsTotal, maxChecksPerDay}`、`trigger.kind="agent"` 只读判定 + `trigger_result`、编译期支撑：目录静态进 prompt + 唯一查询工具 `phone.lookup` + `event_publish` / `publishes` 登记），新工具从 7 个收到 3 个，字段集一页写完；**判定默认原生 `trigger.script`，十二场景里只有 S7 必须过模型**（S8 / S9 视有无结构化 API）；组合条件一种写法（sources + state 时间戳 + nextCheckAt，"或"甚至不需要 trigger）；派生事件只差 payload 一个字段；**六项拍板**：D1 fire = "需要执行一次 payload"（trigger 检测跃迁，S6 分档 / S7 上下沙发一条 job）、D2 脚本运行时边界与"失败 = fire:false"、D3 预授权分期（一期 ask_user，二期参数级 toolsAllow）、D4 跨 session ask_user 强制 announce、D5 入口默认 webhook + 原生 `stream` 零改动验证、D6 activeWindow 窗外仍维护 state；S5 改为编译期问清车次的一条 `at`；分期一期 / 二期、动手前待核实 V1–V6；砍掉 v2.x 的 `multi` / `gate` / `hysteresis` / `budget` / `once` / T1–T3 / 路 A / `event_catalog` + 四个解析工具 / `current`（附录 A10 逐项给理由）；26 页经无头浏览器三宽度逐页校验无溢出 |
