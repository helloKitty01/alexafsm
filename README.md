...
| [docs/openclaw-automation/](./docs/openclaw-automation/) | openClaw 自动化任务整体方案（定时 + 事件订阅，v0.1 幻灯片 11 页）：五场景归类、openClaw cron 现状与 session 四选一、EMS 分工边界、一张 Task 表 + 两个触发引擎 + `/hooks/events` 入口三分支、订阅 / 推送契约与 at-least-once 投递语义、七步编译、派生事件 checker 与抢票特例、工具合一（`create_task` trigger 判别联合）、可靠性清单与开放问题 |
