...
| [docs/photo-search-album/](./docs/photo-search-album/) | 图片搜索 skill × 创建相册 skill 的组合场景（相册单向依赖搜图；搜索工具每次 ≤ 500 张需 cursor 翻页；「一周后同一句话」增量更新需搜索工具加可选 `exclude_album`，排除必须在分页之前）：需求与场景矩阵 S1–S10、两条主流程、工具契约草案、依赖与挑战、三件待拍板；含 v1 幻灯片 6 页 |
