...
| [docs/omni-realtime-voice/](./docs/omni-realtime-voice/) | omni 实时语音架构洞察（三代架构演进与业界谱系、GPT‑Live 全双工 + 说/想分离、OpenAI 六个月实时系统工程：流式推理/实例切换/压缩即切换/WARP/relay+transceiver、Realtime API 会话与事件模型、对双脑架构的启示、全双工输出状态盘点与统一状态模型 v3.5：42 场景全集 × 状态交互图 × 四层分层 × 状态 × 事件完备性矩阵 × 八缺口与五决策 × 词表校订 × 外挂 ASR 标识（v3.5 汇报版幻灯片 22 页：全貌图（正交走线 + 编号光晕，0 遮挡）/ 42 场景分级含状态轨迹 / 完备性矩阵 / 打断判定窗含回声前置 / 四页 12 条共用时间轴的泳道链路走查 / 四层协议 / 与外部词表逐项对比 / 五项决策；v1–v3.4 留档 history/）、LiveKit Agents × OpenAI 集成实现） |
