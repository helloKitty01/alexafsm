...
| [docs/kv-cache-discussion/](./docs/kv-cache-discussion/) | KV cache 命中率与 agent 上下文工程（分层排布、渐进式加载、压缩策略）的设计讨论；含**最终汇报版 `kv cache方案（汇报版）.v3.1`（3 页，给领导：P1 数据基线——优化前 / 优化后两张三列表（序号 · 模块 · 长度），Tools 4→1、动态 SP 内→U 前半两行蓝底，右栏业务占比 47/28/25 与轮数分布；P2 上下文组成对比——沿 v1.4 P2 骨架只换数据：两条结构条 + 当前 / 目标两张 6 行逐次调用矩阵（Q1 问答、Q2 任务含 skill+工具 loop、Q3/Q4 闲聊），右列每行"白算 + 必算 = 实算"，四把尺子（命中率 62%→90%、Q 首调实算 28–36K→3.3K、4Q 会话 136K→39K −71%、三步改造），底部一行账目恒等式；P3 轮数 × 业务——ΔS(1)=5.9、ΔS(n≥2)=14.4+7(n−1)，1–8 轮实算曲线与节省阴影，分桶 6Q+ 按 6 轮加权 −59%，三句话与前提。两边同内容只换排布、高频 skill 不进 SP、动态两边 3K、A 拆 tool call / answer、T 分 webSearch 10 / SkillLoad 5 / 工具 2.5；页内 `BASELINE` 数据绑定，三视口校验通过；v3、升级版 v1 / v2、v1.4 及更早留档 history/，archify 源仍在 archify/）**、v2 汇报大纲（13）、实测数据需求（14）、旧基线（15）、**当前工作基线 16 优化后 / 17 优化前（各一张四列表、长度全为确定数且两边同长）、v3.1 账目与公式底稿（18）**、工作底稿 M1–M9 长度与变化行为表、基线模板 v2（0921 实测 chars，现网中间态）、逐调用实测 `token_breakdown.csv`、prompt 各段内容变化对照，以及 v2 单页幻灯片 P1 组成对比 v1.2 / P2 尺子与三方案 / P7 自动化统计；上一版整合材料 kvcache整体方案.v1 暂留对照 |
