...
| [docs/photo-search-album/](./docs/photo-search-album/) | 图片搜索 skill × 创建相册 skill 的组合场景（相册单向依赖搜图；搜索工具每次 ≤ 500 张；「一周后同一句话」增量更新靠搜索工具可选 `exclude_album`，排除在取数之前）。**当前 v2**：三项拍板——同名默认增量多本才问 / 排除语义 = members ∪ removed 由相册服务维护 / 相册流程模型不翻页：「排除即翻页」替代游标 + 复合工具 `add_from_search` 吞掉循环，首次与增量同链路 3 个 loop 与张数无关；场景矩阵 S1–S11、契约 v2、依赖挑战与待核实 V1–V5；8 页幻灯片（v1 留档 history/） |
