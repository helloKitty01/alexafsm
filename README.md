...
| [docs/llm-choice/](./docs/llm-choice/) | 2026-09-23 模型与 agent 怎么选：API 价、订阅、Artificial Analysis 效果与速度、易用。默认建议是自建用 GPT-6 Sol，难回合升 Opus 5.5，人写代码用 Claude Pro，不把生活代办交给 Muse 外呼。另有收费展开：输入与输出分开计价的原因，Cursor Ultra 里 Fable 的 token 折算，以及「单价与 API 相同、优惠在额度面值」 |
| [docs/agent-radar/](./docs/agent-radar/) | 每日 Agent 雷达（北京时间 0:00）：近一天一手动态 + 近 7 日论文，每份一个日期文件。当前 [2026-09-23](./docs/agent-radar/2026-09-23.md)（手动预跑）：Muse 外呼的人工承包商、ZCode 默上传仓库、ContrAgent 轨迹合同 |
