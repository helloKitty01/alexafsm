...
| [docs/openclaw-automation/](./docs/openclaw-automation/) | openClaw 自动化任务统一方案（定时 + 事件订阅 + 脚本 / 模型判定触发；**当前 v3.2**：单文件 28 页幻灯片（主稿 18 页 + 附录 10 页，P2 为汇报用"一页看懂"）+ `02-event-catalog.md` 事件目录全文 + `archify/` 九张图的 Typed JSON 源，旧版留档 history/）：以 openClaw 原生 `cron` 为唯一中枢按需新增——一条路 **tick → [trigger] → run**，trigger 是 L2 起才有的可选旁路，fire 是 trigger 返回值里的一条定义而不是节点；**限额不是一层**：叫醒窗口 / cooldown 归 `sources[]`（时间源直接写 cron 表达式或 `startAt–endAt`），依赖 run 事实的节流归 trigger（tick 带 `lastRunAt`），`maxRunsTotal / maxRunsPerDay / maxChecksPerDay` 是 cron service 的系统配额（默认值 + 可覆盖，到顶暂停告警，不经判定者）；**新增只有五项**（`sources[]` 多源调度与事件适配器、tick + `nextCheckAt` + `done`、配额计数器、`trigger.kind="agent"` 只读判定 + `trigger_result` + 可选 `postScript`、编译期支撑：目录进 prompt + `phone.lookup` + `event_publish`），新工具三个；**判定者 script 与 agent 同一契约、动作者 payload script 与 agentTurn 同一条成本阶梯**：有结构化数据源默认脚本判定，看图 / 读页面选模型，只有 S7 必须选；动作在编译期能写成固定工具调用 → `payload.script`（默认，零 token，S3 / S7 / S10 / S11 全程不过模型），要生成 / 交互 / 必确认 → agentTurn；D2 脚本边界两档（trigger 只读 / payload 可写但不能 cron / ask_user）；组合条件一种写法（sources + state 时间戳 + nextCheckAt）；派生事件只差 payload 一个字段；**六项拍板** D1–D6；分期一期含两种判定者与两种动作者；待核实 V1–V7。**九张 archify 图**（架构 / 一条路 workflow / agent 判定 workflow / S9 · S7 lifecycle 状态机 / 派生事件与事件入口 dataflow / S3 · S2 sequence）保钩子内联，`→` / `←` 逐章高亮、`P` 自动播放、进入视口 trace，配色映射到原五色不变；三宽度逐页校验无溢出 |
