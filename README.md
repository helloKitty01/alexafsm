...
| [docs/kv-cache-discussion/](./docs/kv-cache-discussion/) | KV cache 命中率与 agent 上下文工程（分层排布、渐进式加载、压缩策略）的设计讨论；含**最终汇报版 `kv cache方案（汇报版）.v3`（3 页，给领导：P1 数据基线——两条物理序结构条 + 模块表 × 当前 / 目标位置与命运 + 运行时零件 + 业务占比与轮数分布 + 四条恒等式，全稿只从这里取数；P2 当前 vs 目标——实算 100K → 36K（−64%）、白算 64K → 0、必算两边 36K、命中率 66% → 88%，三步改造 + Q1 问答 / Q2 任务 / Q3 闲聊逐调用绿红蓝堆叠柱；P3 轮数 × 业务——ΔS(1) = 5.9、ΔS(n≥2) = 14.4 + 7(n−1)，1–8 轮当前 / 目标实算曲线与节省阴影，分桶加权 −63%，6Q+ 占算力 48% 贡献 59%。两边同内容只换排布、高频 skill 不进 SP、动态两边 3K、A 拆 tool call / answer、T 分 webSearch 10 / SkillLoad 5 / 工具 2.5；页内 `BASELINE` 数据绑定，三视口校验通过；升级版 v1 / v2 与 v1.4 及更早留档 history/，archify 源仍在 archify/）**、v2 汇报大纲（13）、实测数据需求（14）、旧基线（15）、**当前工作基线 16 优化后 / 17 优化前（各一张四列表、长度全为确定数且两边同长）、v3 账目与公式底稿（18）**、工作底稿 M1–M9 长度与变化行为表、基线模板 v2（0921 实测 chars，现网中间态）、逐调用实测 `token_breakdown.csv`、prompt 各段内容变化对照，以及 v2 单页幻灯片 P1 组成对比 v1.2 / P2 尺子与三方案 / P7 自动化统计；上一版整合材料 kvcache整体方案.v1 暂留对照 |
