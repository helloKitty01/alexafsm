...
| [docs/openclaw-automation/](./docs/openclaw-automation/) | openClaw 自动化任务统一方案（定时 + 事件订阅，v0.2 幻灯片 12 页，v0.1 留档 history/）：以 openClaw cron service 为唯一中枢按需新增——原生 `cron` 工具全貌（含 `trigger` 条件监视器）、session 四选一、EMS / 原生 / 新增三方归属与 γ 方案、新增 `schedule.kind="event"` 规格与投递语义、事件启动 Agent 的上下文组织（五块信封）、`payload.message` 三段编译模板、三种触发的完整 `cron add` 工具调用实例、派生事件路 A / B、抢票两阶段统一形态、可靠性清单 |
