...
| [docs/kv-cache-discussion/](./docs/kv-cache-discussion/) | KV cache 命中率与 agent 上下文工程（分层排布、渐进式加载、压缩策略）的设计讨论；含**最终汇报版 `kv cache方案（汇报版）.v3.6.1`（3 页，给领导：P1 数据基线——优化前 / 优化后两张三列表（序号 · 模块 · 长度；Tools 5.9 / System Prompt 28.9 / 会话级 1.5 / 历史 4 / U 动态 3.5 / query 0.1 …），Tools 4→1、动态 System Prompt 内→U 前半两行蓝底，右栏业务占比 47/28/25 + 16 桶会话轮数横向柱状图；P2 上下文组成对比——两条结构条 + 三步改造 + 当前 / 目标两张 5 行 × 10 列逐次调用矩阵（检索 / skill 以伪造 A/T 随 U 进入、不占模型调用；问答 1 次、任务 2 次、闲聊 1 次；每行账目带该次调用命中率），底部两行：账目逻辑（输入 = 已缓存 + 白算 + 必算 / 实算即内部口径"硬算" = prefill 真正计算的量 / cache 命中率）与目标衡量（白算 → 0 验收线 89K→0、必算两边相同 38.8K 并分首调必算 3.9 / 9.1 / 14.1K 三档与整轮实算 10.6K、实算 127K→38.8K −70%、命中率全业务加权 55%→86%）；P3 轮数 × 业务——ΔS(n≥2)=11.4+7.1(n−1)，Q1–Q6 实算曲线与每轮命中率（当前 65%→47%、目标 75%→92%），分桶 1Q–5Q / 6–10Q / 11Q+（≥11 轮按 12 轮计）加权 −65%、分桶表带 cache 命中率列（加权 55%→86%），首调 TTFT 预估表（46 ms/K：Q2–Q12 1.0–4.3 s → 180 ms，预热后统一 170 ms，业务必带问答 +470 / 任务 +240 ms），三句话 + 效果"会话数 10% / 算力 54% 迷你横条 / 会话级可提前算 / 白算归零是验收线 / 效果不变"。两边同内容只换排布、页内 `BASELINE` 数据绑定，三视口校验通过；v3.6 及更早留档 history/，archify 源仍在 archify/）**、v2 汇报大纲（13）、实测数据需求（14）、旧基线（15）、**当前工作基线 16 优化后 / 17 优化前、v3.6.1 账目与公式底稿（18）**、工作底稿 M1–M9 长度与变化行为表、基线模板 v2（0921 实测 chars，现网中间态）、逐调用实测 `token_breakdown.csv`、prompt 各段内容变化对照，以及 v2 单页幻灯片 P1 组成对比 v1.2 / P2 尺子与三方案 / P7 自动化统计；上一版整合材料 kvcache整体方案.v1 暂留对照 |
