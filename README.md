...
| [docs/photo-search-album/](./docs/photo-search-album/) | 图片搜索 skill × 创建相册 skill 的组合场景（相册单向依赖搜图；搜索工具每次 ≤ 500 张；「一周后同一句话」增量更新靠搜索工具可选 `exclude_album`，排除在取数之前）。**当前 v2.1**：当前方案 → 目标方案同图对照（四处变化：`exclude_album` / 复合工具 `add_from_search` / 相册服务 `removed` / 引擎排除下推）+ 五个优化点；三项拍板——同名默认增量多本才问 / 排除语义 = members ∪ removed / 相册流程模型不翻页、循环进复合工具，首次与增量同链路 3 个 loop 与张数无关；**v2.1 修正**：exclude 定义增量集合、游标负责运行内定位、引擎能读己之写时两者合一——四页索引时效性专题（T_sync 时序与静默少加 → 排除集烘进索引 vs 查询时反连接 → 工具内三策略决策表，推荐 exclude 播种 + 游标定位 → 影响与延迟注入验收）；场景矩阵 S1–S11、契约 v2.1、待核实 V1–V6；14 页幻灯片（v1 / v2 留档 history/） |
