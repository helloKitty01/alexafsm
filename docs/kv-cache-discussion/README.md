...
| [13-汇报大纲.md](./13-汇报大纲.md) | **v2 整合汇报大纲**:已确认口径(三层物理块、四级分级、会话级横跨 SP/UAT、历史完整不折叠、会话首调必算)+ 页序 P1 组成对比 → P2 分级与改造四句 → P3 命中率模型 → P4–P6 分场景 → P7 汇总 → P8 统计闭环 → P9 会话级预热 → 附录,每页要点/数据依赖/状态 + 待补数据清单 + 术语表 |
| [14-上下文组成实测数据需求.md](./14-上下文组成实测数据需求.md) | 填充 P1 虚框与 P3–P9 参数的数据需求(12 的续篇,按四级分级重定义模块):A 产品级(Tools 清单与变体 / SP 固定构成与伪静态 diff / 预置 skill 覆盖率)、B 会话级(SP 块 / 历史 10 轮)、C/D query 与 loop 级各消息长度、E 会话形态与引擎侧(cached_tokens、prefill 吞吐、ASR 时长);统一口径 + JSON 填写模板 + 优先级 |
| [上下文组成对比.v1.1.slides.html](./上下文组成对比.v1.1.slides.html) | **v2 汇报 P1(单页,当前版本 v1.1)**:目标态物理序 Tools → SP → UAT(Tools 成为产品级、会话首调即命中);上半当前/目标两根结构条按产品级/会话级/query级/loop级着色(虚框 = 待实测);下半"行 = 调用、列 = 位置"阶梯矩阵把 UAT 按 Q 与 loop 展开(3Q · 6 次调用),格色 = 命中/原位重写/被连带/新增/会话首调一次;读法 + 改造四句 + 待补清单。v1(目标态 Tools 位于 SP 之后)留档 history/ |
| [kvcache整体方案.v1.slides.html](./kvcache整体方案.v1.slides.html) | **整合汇报材料(9 页,当前版本 v1)**:P0 整体收益总览("白算的 token 才是成本",版式源自收益闭环 v4、数字为 v7.2 口径)+ P1–P7 收益闭环(v7.2 全量七页)+ P8 统计点位与自动化(v1.2 单页) |
...
被新版取代的幻灯片全部留档在 [history/](./history/) 子目录，共四个系列：
