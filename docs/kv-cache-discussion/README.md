...
| [kv cache方案（汇报版）.v1.1.slides.html](./kv%20cache方案（汇报版）.v1.1.slides.html) | **最终汇报版承载(8 页,当前 v1.1)**:v1.1 锁定 P1 两项决策——目标 Tools = 全量 12 个工具全部预置(5.9K,不再"清单待定")、预置 skill 取 15 个高频(按实测 .25–.4K/条估 4–6K),结构条改为"示意、不按比例",分级保持四级;P3 方案 B / P8 待确认同步。按 13 大纲页序,P1 组成对比(虚框用 CSV 实测回填)→ P2 已上线第一步各段内容对照(prompt段内容变化对照.md)→ P3 两把尺子 + 三方案(实测锚点)→ P4 闲聊三列账(当前 / 中间态实测 / 目标)→ P5 问答 + 任务 9 次调用走查(白算 ≈ miss − Δ输入)→ P6 整体收益(v7.2 公式与分桶,系数实测校核)→ P7 自动化统计(块粒度 / 冷启标签)→ P8 结论与待确认。替代 kvcache整体方案.v1;v1 留档 history/ |
