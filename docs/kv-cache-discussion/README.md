...
| [上下文组成对比.v1.slides.html](./上下文组成对比.v1.slides.html) | **v2 汇报 P1(单页,当前版本 v1)**:上半当前/目标两根结构条按产品级/会话级/query级/loop级着色(虚框 = 待实测);下半"行 = 调用、列 = 位置"阶梯矩阵把 UAT 按 Q 与 loop 展开(3Q · 6 次调用),格色 = 命中/原位重写/被连带/新增/会话首调一次;读法 + 改造四句 + 待补清单 |
