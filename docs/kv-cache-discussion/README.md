...
| [kv cache方案（汇报版）.v1.2.slides.html](./kv%20cache方案（汇报版）.v1.2.slides.html) | **最终汇报版承载(8 页,当前 v1.2)**:v1.2 重做 P1——两行结构条按"级 + 内容"统一命名并六块一一对位(当前 SP·会话级 = user.md 占位 + device .3–.6K ↔ 目标 SP·会话级 ~1.0K 含记忆快照;当前 SP·动态⚡ 3.0K ↔ 目标 UAT·动态增量 ≤2K 单独成段),预置 skill 定 15 个 · 4.5K,SP·固定加 14 段概述(~45% 工具与技能规范 / 20% AGENTS.md / 其余人格风格与回复合成),底部改为三尺子对比条(命中率 65.5% → ≥95%、硬算 17.2K → ~2.3K/调用、4Q 闲聊会话 54K → ~19K)+ 改造四句;矩阵目标侧 U 列改 U+Δ。v1.1 锁定目标 Tools = 全量 12 个工具全部预置(5.9K)、四级、不按比例。按 13 大纲页序,P1 组成对比 → P2 已上线第一步各段内容对照(prompt段内容变化对照.md)→ P3 两把尺子 + 三方案(实测锚点)→ P4 闲聊三列账(当前 / 中间态实测 / 目标)→ P5 问答 + 任务 9 次调用走查(白算 ≈ miss − Δ输入)→ P6 整体收益(v7.2 公式与分桶,系数实测校核)→ P7 自动化统计(块粒度 / 冷启标签)→ P8 结论与待确认。替代 kvcache整体方案.v1;v1 / v1.1 留档 history/ |
