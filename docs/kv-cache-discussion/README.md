...
| [kv cache方案（汇报版）.v1.4.slides.html](./kv%20cache方案（汇报版）.v1.4.slides.html) | **最终汇报版承载(9 页 + 附录 3 页,当前 v1.4)**:v1.4 新增 **P1 数据基线页**(M1–M11 模块 × 当前 / 目标长度 × 状态,K1–K4 常量,R1–R6 规则;与 15 同源),全稿长度 / 目标 miss / 节省率由页内 `BASELINE` + `SESSIONS` 对象计算并 `data-b` / `data-calc` 绑定,改基线即全稿同步;P3 "三方案"改为与 P2 四句同序的**四步改造**(① 动态下沉已上线 ② Tools 前移 ③ skill 预置 ④ 只更新变化,预热不进清单),device 统一归会话级,动态基线统一 3.0K → ≤2K;P4–P6 账目页改为数据渲染(Q 边界按 U 列变化 K4,任务 / 问答各含 1 次 loop 全 miss、目标归零;三条实测会话合计 −73% / −79% / −82%),修掉标签重叠、条纹低对比、页脚与翻页键重合。v1.3 按 13 大纲 / v7.2 模板重排全稿——P1 组成对比(去预热口径;4Q 闲聊尺子改为 54K → ~10K,−81%)→ P2 两把尺子 + 三方案(去中间态列,改为"目标 / 目标的实测校核";方案重排为 A 高频 skill 预置、B 只更新变化、C 会话级预热放最后且不进账本)→ **P3 任务型 / P4 问答型 / P5 闲聊型三页逐 Q、逐调用账目(v7.2 版式,当前实测 vs 目标推演同位对照,每 Q 给省额与比例;三条实测会话合计 −73% / −80% / −81%)**→ P6 整体收益(对账表改为三类 Q 首调实测 vs v7.2 推演)→ P7 自动化统计(scheme 只保留当前 / 目标)→ P8 结论(KPI 改为当前 → 目标口径);"中间态"不再作为主线一列,已上线第一步的实测下沉为**附录 A1(各段内容对照)/ A2(问答 + 任务 9 次调用审计 + 对 P3–P5 目标列的三点校核)**,作为目标推演的证据;**附录 B 方案 C 会话级预热**(只影响首调 TTFT,三类会话首调 当前 / 目标 / 预热后 对照)。v1.2.1 修正 P1 矩阵语义;v1.2 重做 P1(两行结构条对位命名、三尺子对比条);v1.1 锁定目标 Tools = 全量 12 个预置、预置 skill 15 个 4.5K。替代 kvcache整体方案.v1;v1 / v1.1 / v1.2 / v1.2.1 / v1.3 留档 history/ |
| [kvcache整体方案.v1.slides.html](./kvcache整体方案.v1.slides.html) | 上一版整合汇报材料(9 页,已被"kv cache方案（汇报版）"取代,暂留作对照):P0 整体收益总览("白算的 token 才是成本",版式源自收益闭环 v4、数字为 v7.2 口径)+ P1–P7 收益闭环(v7.2 全量七页)+ P8 统计点位与自动化(v1.2 单页) |
...
| kv cache方案（汇报版） | v1、v1.1、v1.2、v1.2.1、v1.3 | v1.4 |
