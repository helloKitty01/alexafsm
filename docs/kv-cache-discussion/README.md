...
| [kv cache方案（汇报版）.v1.slides.html](./kv%20cache方案（汇报版）.v1.slides.html) | **最终汇报版承载(8 页,当前 v1)**:按 13 大纲页序,P1 组成对比(虚框用 CSV 实测回填)→ P2 已上线第一步各段内容对照(prompt段内容变化对照.md)→ P3 两把尺子 + 三方案(实测锚点)→ P4 闲聊三列账(当前 / 中间态实测 / 目标)→ P5 问答 + 任务 9 次调用走查(白算 ≈ miss − Δ输入)→ P6 整体收益(v7.2 公式与分桶,系数实测校核)→ P7 自动化统计(块粒度 / 冷启标签)→ P8 结论与待确认。替代 kvcache整体方案.v1 |
