...
| [openClaw自动化任务方案.slides.v3.3.html](./openClaw自动化任务方案.slides.v3.3.html) | **v3.2 内容的整页图版（35 页 = 主稿 23 页 + 附录 12 页），与 v4 并列**。内容一字不改（仍是 `sources[]` / `quota` / `done` / `lastRunAt` 那套通用模型，页码顺延：旧 P6 → P7、P10 → P12、P11 → P13、P13 → P16、P17 → P21、P18 → P23），只做两件事：① **九张 archify 图各占一页**（`.slide.figure`：页头一句 lede + 图占满 + 章节栏），原来挤在图旁的名词表 / 对比护栏 / S9 脚本与 tick 表 / run 状态机 / A9 传输表 / A10 JSON 拆成紧邻的文字页并把字号放回 .74–.9em；② **悬停流光与钉住**（archify Intent Trace 移植：悬停节点压暗非相邻、相邻边跑光点、出边蓝 / 入边紫；悬停连线亮两端，边有 14px 命中副本；点击钉住并在章节栏显示上游 / 下游，再点或 Esc 取消，切章自动取消）。保留进入视口 trace、`→` / `←` 逐章、`P` 播放，配色不变。三宽度 35 页校验无溢出。用途：向不接受 v4 收敛的读者展示 v3.2 的完整设计；v4 定稿后可归档 |
