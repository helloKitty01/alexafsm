...
### 4.6 v3.3:把 4.5 的整页图 + 悬停层搬到 v3.2 原内容上(拆页与页码重映射的实现要点)

v3.3 与 v4 是同一套动效的两条线:v4 换了内容,v3.3 内容 = v3.2、只改版式与动效,供并列比较。补几条 4.5 没写的实现细节:

- **拆页**:原"左图右卡"页拆成"图页 + 紧邻文字页"(P4 图 / P6 图 + P7 名词表 / P11 图 + P12 对比护栏 / P13 文字 + P14 图 / P16 文字 + P17 图 / P21 图 + P22 run 状态机 / A5 图 + 底部三格短注 / A9 文字 + 图 / A10 JSON + 图),28 页 → 35 页。文字页字号顺势回到 .74–.9em;图页 subtitle 写图型 · 规模 · 章节顺序 · 怎么操作。
- **页码重映射**:主稿 P 号用正则 `(?<![A-Za-z0-9#=_./-])P(\d+)(?!\d)` 按映射表替换;**先把 `<!-- ═══ P6 … -->` 分节注释换成占位符再映射**,否则后续按旧标题找锚点会失败;附录 A 号不动;页脚 `x / N` 先写占位符、拼装完再顺序回填;封面目录与阅读路线手写。
- **边的命中副本**:边本身 1.5px 悬不到,初始化时按 `path[data-edge-id][d]` 给每条边加一条 14px 透明、`pointer-events: stroke` 的 `.it-hit` 副本放在最上层;悬停它 = 悬停边(该边 path + 标签 `g` + 两端节点亮起,光点用第三色)。
- **钉住与分章的关系**:切章(`setChapter`)时先取消钉住;钉住状态下光点改成 1.6 s 无限循环;章节栏文字写"钉住 X · 上游 N:A(边标签)、B · 下游 M:…",取消后恢复章节文字。
- **测悬停边**:不能用 `locator.hover()`(L 形路径 bbox 中心不在描边上),用 `getTotalLength / getPointAtLength` 取描边上一点、乘 `getScreenCTM` 后 `mouse.move`。
- **已知限制**:lifecycle 主轨的隐式前进箭头没有 `data-edge`,悬停主轨状态只亮显式转移;sequence 的时间段框不参与悬停。
