...
P2 整体方案（一张分层架构图标清**已有 / 新增 / 待决策**：新增只有三项——searchTool 的 filter 支持 `album` 字段即 `album not in [A]` 表达排除 + 输出 total、appendAlbum 增加 `source=search` 形态在工具内吞掉循环、相册整理 skill 改写不再教翻页；主流程首次与增量同一条 3 个 loop）；
P3 工具定义（searchTool 输入 query / showCard / cursor | offset / pageSize / filter 与输出 items / hasMore / nextCursor / total 逐项标已有・新增，三段调用示例，filter 必须在取 pageSize 之前下推、`album` 字段 = 成员 ∪ 移出记录；appendAlbum 两种形态；createAlbum；找册）；
P4 关键细节（offset 必错 / 只靠排除的前提 / 排除播种 + 游标定位；推荐写法；索引更新不及时的时序与静默少加；引擎能力 → 写法决策表）；
P5 决策 D1–D4（推荐 / 备选 / 取决于）、核实 V1–V5、验收加两条延迟注入用例、风险排序、下一步。
...
| [图搜建册方案.slides.v4.html](./图搜建册方案.slides.v4.html) | 5 页汇报版（浅色 16:9）：背景与场景 / 整体方案（单图标已有・新增・待决策）/ 工具定义（searchTool 输入输出、appendAlbum 两种形态）/ 关键细节 / 决策・核实・验收 |
