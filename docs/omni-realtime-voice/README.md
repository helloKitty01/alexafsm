...
| [全双工输出状态.slides.v3.2.html](./全双工输出状态.slides.v3.2.html) | **汇报版 v3.2（15 页，中文；状态名统一为「英文（中文）」，事件名保留协议字段）**：封面（含议程）→ 人机六差异逼出机制 → 一张图看全貌（主干加粗、边编号①–⑯、扩展态独立节点）→ 完整状态清单（系统 11 / 用户 4 / floor 五值）→ 三条设计原则与三分类 → 一轮对话五段 → **34 场景全部覆盖且每条带状态轨迹**（9 必做 / 15 分水岭 / 10 暂缓含理由 + 双向覆盖检查）→ 打断三分判定 + 指标 + 策略取向 → PREPARING/HOLDING 两套阈值与话术示例 + 工具委托 → 四层协议 + 产地决策 + 外挂 ASR 信号 → **与外部 7 项词表逐项详细对比** → 三项决策（含影响范围）+ 五条模型需求 + 两条验收 + 下一步 → 附录完整转换表（16 + 6） |
| [全双工输出状态.slides.v3.html](./全双工输出状态.slides.v3.html) | **技术讨论底稿 v3（11 页）**：34 场景**逐条细化到状态轨迹**（A/B/C/D/X 五组）+ 双向覆盖检查（状态↔场景）→ **完整状态交互图**（含入口链与议题标注）→ 两条泳道走查（B3 停顿不抢答、D3 真打断全链路）→ 转换表（16+6，议题挂行）→ **三个待讨论议题**（HOLDING 续说时延归属 / REPAIR·BACKCHANNEL 的存在理由 / T1 filler 出声归属）；结论收敛后回写 08 章 |
| [history/](./history/) | 历史版本留档：全双工输出状态幻灯片 v1（11 页，10 态讨论稿版）、v2（12 页，场景全集 × 四层协议汇报版）、v3.1（16 页，英文为主的汇报版初稿），均已被 v3 / v3.2 取代 |
