...
| [08-场景全集与状态交互分层.md](./08-场景全集与状态交互分层.md) | 统一状态模型 **v2 → v3.7 修订**（场景驱动）：**44 场景全集**（主序 A→B→C→D→X，**P0/P1/P2 为优先级列不是目录**：P0 最小可用 10 / P1 体验分水岭 23 / P2 增强暂缓 11；v3.3 矩阵补入 B10、D11–D14、X5–X7；v3.6 全集展开补入 C8 工具失败降级、D15 长回答分段）；三分类原则；`THINKING/DELEGATING` 拆分为 `PREPARING` + `HOLDING`；工具 / 委托不建态、时延 T0/T1 状态级 + T2 轮级（T1 垫话模型自发、系统兜底）；四层分层（判定归 omni 模型、事实归端侧与任务层、状态归归约器）；外部 7 项事件词表校订；外挂 ASR 外部标识（+ `segment_merge`）；对模型输出五条需求与六场景验收 + C5/D7 六行为用例组；**第 10 节（状态·floor）× 事件完备性矩阵**；**第 11 节九个已识别缺口 + 决策 1–5 + C4 × D3 链路走查（v3.5：带时间轴 + 泳道五条对齐不变量 + 12 条走查索引）**（v3.4：C5 / D7 统一为"系统持轮时用户开口"三条不判意图的规则） |
| [全双工输出状态.slides.v3.7.html](./全双工输出状态.slides.v3.7.html) | **汇报版 v3.7（23 页，中文；状态名统一为「英文（中文）」，事件名保留协议字段；绿色 NEW 标记 v3.3–v3.7 新增）**：封面（含议程）→ 人机六差异逼出机制 → **一张图看全貌（v3.4 重画：正交走线、边只标带白底光晕的编号 ①–⑯、图例置于图下）**→ 完整状态清单（系统 11 / 用户 4 / floor 五值）→ 三条设计原则（T1 模型自发、系统兜底）→ 一轮对话五段（34 → 44）→ **分叉树** → **44 场景按 A→X 主序四页展开**（色标为 P0/P1/P2，红底为命门 B3/D3/D14；不再按批次分页）→ **场景全集矩阵**（格子 → 出口 → 场景；红格 = C8/D15/A5 规则③）→ 打断判定窗（回声前置 + 真打断 / 暂停请求 / 附和 / 噪声四出口 + 策略取向）→ PREPARING/HOLDING 阈值 + 轮级时钟 + 工具委托 → **四页 12 条泳道链路走查（v3.5：四行共用秒级时间轴）** → 四层协议 + 谁产谁消 + 外挂 ASR 信号 → 与外部 7 项词表逐项对比 → **九个已识别缺口及处置** → **五项决策** + 五条模型需求 + 六场景验收 + 六行为用例组 + 下一步 → 附录完整转换表。v3.6 起补入 C8/D15、生产者/消费者表与 T1 模型自发系统兜底 |
| [history/](./history/) | 历史版本留档：全双工输出状态幻灯片 v1（11 页，10 态讨论稿版）、v2（12 页，场景全集 × 四层协议汇报版）、v3（11 页，技术讨论底稿：轨迹 + 两条泳道 + 三议题）、v3.1（16 页，英文为主的汇报版初稿）、v3.2（15 页，中文汇报版，34 场景 / 三项决策）、v3.3（19 页，42 场景 / 矩阵 / 三泳道 / 五决策，全貌图为曲线走线）、v3.4（19 页，全貌图正交重画 + C5/D7 三规则，泳道仍为手填宽度）、v3.5（22 页，12 条共用时间轴泳道）、v3.6（23 页，全集矩阵 + 分叉树 + C8/D15，场景表仍按 P0/P1/P2 分页），均已被 v3.7 取代 |
