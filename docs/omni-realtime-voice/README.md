...
| [全双工输出状态.slides.v3.1.html](./全双工输出状态.slides.v3.1.html) | **汇报版 v3.1（16 页，英文为主 / 中文括注）**：封面一句话结论 → 议程 → 人机六差异逼出机制 → 一张图看全貌（主干加粗）→ 完整状态清单（系统 11 / 用户 4 / floor）→ 三条设计原则 → 一轮对话五段 → **34 场景全部覆盖**（9 必做 / 15 分水岭 / 10 暂缓，含暂缓理由与双向覆盖检查）→ 打断三分判定与策略取向 → PREPARING/HOLDING 时延三档与工具委托 → 四层协议 + 控制面词表 + 外挂 ASR 信号 → **与外部 7 项词表逐项详细对比**（逐条裁定 + 结构性缺口 + 可取之处）→ 三项待拍板决策 + 五条模型需求 + 两条验收 + 下一步 → 附录完整转换表（16 + 6） |
